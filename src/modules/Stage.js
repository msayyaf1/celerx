import {Point, Graphics, Container, loader, extras, Sprite, Texture} from 'pixi.js';
...
const MAX_X = 600; //800, 720
const MAX_Y = 800; //600, 1280
...
const HUD_LOCATIONS = {
  SCORE: new Point((MAX_X/2), 30),
  WAVE_STATUS: new Point(MAX_X - 10, MAX_Y - 20),
  GAME_STATUS: new Point(MAX_X / 2, MAX_Y * 0.40),
  REPLAY_BUTTON: new Point(MAX_X / 2, MAX_Y * 0.52),
  SHARE_BUTTON: new Point(MAX_X / 2, MAX_Y * 0.64),
...
class Stage extends Container {
  ...
  constructor(opts) {
    ...
    // This will add an empty Texture to the game Stage to catch clicks
  ...
  static shareButtonLocation() {
    return HUD_LOCATIONS.SHARE_BUTTON;
  }
  ...
  _setStage() {
    ...
    const bgpos ={};
    bgpos.x = 0;
    bgpos.y = MAX_Y - background.height;

    background.position.set(bgpos.x, bgpos.y);

    const tree = new extras.AnimatedSprite([loader.resources[this.spritesheet].textures['scene/tree/0.png']]);
    tree.position.set(100, (MAX_Y - tree.height)-80);
  ...
  clickedReplay(clickPoint) {
    return Utils.pointDistance(this.getScaledClickLocation(clickPoint), HUD_LOCATIONS.REPLAY_BUTTON) < 50;
  }

  clickedShare(clickPoint) {
    return Utils.pointDistance(this.getScaledClickLocation(clickPoint), HUD_LOCATIONS.SHARE_BUTTON) < 50;
  }
